...
func (server *Server) initializeRoutes() {
	...
	server.Router.HandleFunc("/user/{id}", middlewares.SetMiddlewareAuthentication(middlewares.SetMiddlewareAuthentication(server.DeleteUser))).Methods(constant.HTTPMethodDelete)
	server.Router.HandleFunc("/user/active/{id}", middlewares.SetMiddlewareAuthentication(middlewares.SetMiddlewareAuthentication(server.ToggleUserIsActive))).Methods(constant.HTTPMethodPut)
	...
	server.Router.HandleFunc("/loan-type/{id}", middlewares.SetMiddlewareAuthentication(middlewares.SetMiddlewareAuthentication(server.DeleteLoanType))).Methods(constant.HTTPMethodDelete)
	...
	server.Router.HandleFunc("/loan-general/{id}", middlewares.SetMiddlewareAuthentication(middlewares.SetMiddlewareAuthentication(server.DeleteLoanGeneral))).Methods(constant.HTTPMethodDelete)
